...
def get_task_template(request):
    ...
    # for t in irrelevant_tasks:
        # print t+'ddddddddddddddddddddddddddddddddddd'
        # tasks.remove(t)
    # print tasks
...
class Check_Periodic_Task(object):
    ...
    def check_crontab(self):
        ...
        try:
            # if long(self.crontab) not in [c['id'] for c in crons]:改动
            if int(self.crontab) not in [c['id'] for c in crons]:
...
def test_periodic_task_result(request):
    ...
    msg = [_r.rpop(_random) for i in range(_r.llen(_random))]
